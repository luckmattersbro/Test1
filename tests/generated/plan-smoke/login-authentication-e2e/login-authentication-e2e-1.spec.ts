...
test.describe('Login / Authentication (E2E)', () => {
  ...
  test('Smoke: Successful login', { tag: '@smoke' }, async ({ page }) => {
    ...
    await expect(page.locator("input#login-username").first()).toBeVisible({ timeout: 10000 });
    await page.locator("input#login-username").first().fill('admin');
    // Fill the password field with the configured test password
    await expect(page.locator("input#login-password").first()).toBeVisible({ timeout: 10000 });
    await page.locator("input#login-password").first().fill('Admin2026#');
  ...
  test('Negative: invalid credentials show error or stay on login page', { tag: '@smoke' }, async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('domcontentloaded');
    const user = page.locator("input#login-username").first();
    const pass = page.locator("input#login-password").first();
